...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
         ...
         */
        if(boundRect[i].width - boundRect[i].height > 15
        || boundRect[i].width - boundRect[i].height < -15
        || boundRect[i].width * boundRect[i].height < 20)               
            continue;                                                   

        /* 
         * crop and resize to 70 X 70, to remove any vestiges of
         * the borders
         */
        boundRect[i].x += 2;                                            
        boundRect[i].y += 3;                                            
        boundRect[i].width = 69;
        boundRect[i].height = 69;
    ...
    for(size_t i = 0; i < selectedRects.size(); i++)
    {
        ...
        resize(imgWrite, usedToWrite, Size(65, 65));
        imwrite(temp, usedToWrite);     
    }
    return 0;
}
